...
from hooks.inference_hook import InferenceHook
from hooks.eval_hook import EvalHook
...
def parser_args():
    parser = ConfigArgumentParser(description="Example")
    parser.add_argument(
        "--work-dir",
        type=str,
        default="../work_dir",
...
def build_dataset(dir):
    ...
    test_dataset = torchvision.datasets.MNIST(dir, train=False, transform=transform)
...
def main():
    ...
    lr_scheduler = optim.lr_scheduler.StepLR(optimizer, step_size=1, gamma=args.gamma)
    # 4. Create Trainer
    trainer = Trainer(
        model,
        optimizer,
