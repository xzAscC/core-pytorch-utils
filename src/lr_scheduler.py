...
class LRWarmupScheduler:
    ...
    def __init__(
        ...
        Args:
            ...
            epoch_len (int): The number of iterations in an epoch. Required only when ``by_epoch=True and warmup_by_epoch=False``.
            warmup_step (int): How many iterations / epochs in warmup stage. If ``warmup_by_epoch=True``,
                "warmup_step" means epoch, else iteration. Defaults to 0 to disable warmup.
        ...
        if warmup_by_epoch:
            assert by_epoch
        if by_epoch and warmup_step and not warmup_by_epoch:
        ...
        if warmup_step:
            ...
            max_step = warmup_step // epoch_len if by_epoch and not warmup_by_epoch else warmup_step
            self.regular_lrs_per_t = self._pre_compute_regular_lrs_per_t(max_step)
    ...
    def epoch_update(self, metric: Optional[float] = None) -> None:
        ...
        self.last_epoch += 1
        if self.warmup_by_epoch and self.last_epoch < self.warmup_step:
            self._set_lrs(
                self._get_warmup_lrs(self.last_epoch, self.regular_lrs_per_t[self.last_epoch]))
        elif self.warmup_by_epoch and self.last_epoch == self.warmup_step:
    ...
    def iter_update(self) -> None:
        ...
        self.last_iter += 1
        if self.last_iter < self.warmup_step:
        ...
        elif self.last_iter == self.warmup_step:
            self._set_lrs(self.regular_lrs_per_t[-1])
        else:
            self.in_iter_warmup = False
            if not self.by_epoch:
